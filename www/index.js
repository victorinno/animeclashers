...
const new_player = () => {
    let player_str = wasm.Player.convert(wasm.Player.new(document.getElementById("name_new_form").value));
    window.localStorage.setItem("player", player_str);
    player = JSON.parse(player_str)
...
function load_clashers() {
    return JSON.parse(window.localStorage.getItem("clashers")) || [];
}
...
const renderLoop = () => {
    if (curr_area == "player_area") {
        ...
        if (!clasher_list_compiled) {
            let clashers = load_clashers();
            const ul = document.getElementById("list_of_clashers");
            while (ul.firstChild) {
                ul.removeChild(ul.lastChild);
            }
            clashers.forEach((clasher) => {
                let li = document.createElement("div");
                li.innerHTML = clasher.name + " - " + clasher.clazz.name + "   ";
                let select_button = document.createElement("button");
                select_button.id = "id_" + Math.random();
                select_button.innerHTML = "Select";

                li.appendChild(select_button);
                ul.appendChild(li);
                document.getElementById(select_button.id).onclick = () => {
                    curr_clasher = clasher;
                    curr_area = "clasher_area";
                };
            });
        ...
        document.getElementById("create_clasher_btn").onclick = () => {
            ...
            let clasher_str = wasm.Clasher.convert(wasm.Clasher.create_clasher(
                document.getElementById("name_new_clasher_form").value,
                wasm.Class.get_by_name(clazz.name)
            ));

            let clashers = (window.localStorage.getItem('clashers')||[]);
            clashers.push(JSON.parse(clasher_str));
            window.localStorage.setItem('clashers', JSON.stringify(clashers));
...
function set_player_name() {
    ...
    document.getElementById("player_name_h").innerHTML = name;
}
